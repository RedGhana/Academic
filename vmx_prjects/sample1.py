...
if vmx.IsOpen(): #VMXCheck (Success == True)
    ...
    try:
        while True:            
            ...
            print(f"Yaw: \t{diff_yaw(default_yaw)}\n\n")
            
            ###MainProcess
            if mode_state == 0:
                if motor1_count < 1000 and motor2_count < 1000:
                    forward(50)
                elif motor1_count < 1000:
                    r_forward(50)
                    l_stop()
                elif motor2_count < 1000:
                    l_forward(50)
                    r_stop()
                else:
                    stop_motor()
                    print("-- Moved to TargetPosition --")
                    vmx.getTime().DelaySeconds(3)
                    mode_state = 1
            elif mode_state == 1:
                if diff_yaw(default_yaw) < -0.5:
                    print("-- Left (Duty: %.02f%%) --" %float((abs(diff_yaw(default_yaw)) + 60) * 0.416))
                    turn_left((abs(diff_yaw(default_yaw)) + 60) * 0.416)
                elif diff_yaw(default_yaw) > 0.5:
                    print("-- Right (Duty: %.02f%%) --" %float((abs(diff_yaw(default_yaw)) + 60) * 0.416))
                    turn_right((abs(diff_yaw(default_yaw)) + 60) * 0.416)
                else:
                    stop_motor()
                    print("-- DefaultYaw --")
                    vmx.getTime().DelaySeconds(3)
                ...
                if motor1_count >= 0 and motor2_count >= 0:
                    back(50)
                elif motor1_count >= 0:
                    r_back(50)
                    l_stop()
