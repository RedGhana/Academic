        ...
        while True:
            ...
            # 物体検出
                ...
                if wait_counter == 0:
                    # print("検出中...")
                    pass
                if wait_counter == 100:  # 機体が静止するまで待つ
                    wait_counter = 0
                    # 入力画像の読み込み
                    camera = cv2.VideoCapture(0)
                    _, frame = camera.read()
                    ...
                    if circle_x is not None:
                        ...
                        print("\n\47nX: %d" %(width/2 - circle_x))
                        print("IMU: %.2f" %vmx.getAHRS().GetYaw())
                ...
                if wait_counter == 100:
                    ...
                    if circle_r < max_size:
                        if circle_x > (center + 20):
                            ...
                            dual_motor(((20 / (center - 20)) * diff_center) + 15, 50)
                            pass
                        elif circle_x < (center - 20):
                            # print("左Duty: %.2f\t右Duty:%.2f" %(50, ((20 / (center - 20)) * circle_x) + 15))
                            dual_motor(50, ((20 / (center - 20)) * circle_x) + 15)
                        else:
                            forward(40)
                    else:
                        stop_motor()
