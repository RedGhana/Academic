...
def init_motor(hum_hicurrdio_output_channels):
    for dio_channel_index in range(0, hum_hicurrdio_output_channels, 1):
        set_pwm(dio_channel_index, 0, 1)    #PWM ON  (Duty 1%)
        set_pwm(dio_channel_index, 0, 0)    #PWM OFF (Duty 0%)
...
def right(duty):
    motor(0, 'forward', duty)
    motor(1, 'forward', duty)
    motor(2, 'forward', duty / 3)
    motor(3, 'forward', duty / 3)
def left(duty):
    motor(0, 'forward', duty / 3)
    motor(1, 'forward', duty / 3)
    motor(2, 'forward', duty)
    motor(3, 'forward', duty)
def turn_right(duty):
    motor(0, 'forward', duty)
...
if vmx.IsOpen(): #VMXCheck (Success == True)
    ...
    try:
        while True:
            ...
            yaw = diff_yaw(target)
            if yaw < -1:
                print("-- Left (Duty: %.02f%%) --" %float((abs(yaw) + 50) * 0.44))
                turn_left((abs(yaw) + 50) * 0.44)
            elif yaw > 1:
                print("-- Right (Duty: %.02f%%) --" %float((yaw + 50) * 0.44))
                turn_right((yaw + 50) * 0.44)
            else:
                print("-- Forward --")
                forward(50)
